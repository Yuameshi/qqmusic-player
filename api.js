...
function throughname() {
    ...
    if (method == true) {
      ...
      alert('即将打开一个新窗口，请在新窗口内选中所有字符并拷贝至稍后的输入框内，若不正确输入则可能出现无法正确播放，歌曲信息无法正确显示等错误！');
      window.open(newwindow,'newwindow','height=700,width=600,top=0,left=0,toolbar=no,menubar=no,scrollbars=no, status=no')
      setTimeout(function(){var getted_json = prompt("请在此输入你接受到的JSON字符串：","");qmjsonstr(getted_json);},5000);
    }
    if (method == false) {
